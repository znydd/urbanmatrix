...
import { fetchGasBillInfo, payMonthlyGasBill } from '@/utils/paybillApi';
...
export default function GasBill() {
    ...
    const getBillInfo = async () => {
        const respBillInfo = await fetchGasBillInfo()
        if (respBillInfo) {
    ...
    const handlePayment = async () => {
        const resp = await payMonthlyGasBill(month)
                ...
                <Card className="w-full min-h-screen mx-auto px-10">
                    <CardHeader className="flex flex-col sm:flex-row sm:justify-between sm:gap-4 sm:items-center sm:space-y-0">
                        <CardTitle className="text-2xl font-bold">
                            <Link href={"/dashboard"} className=' text-teal-500'>Dashboard</Link> / <Link href={"/paybill"}>Pay Bill</Link> / Gas
                        </CardTitle>
