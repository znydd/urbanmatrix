...
import { fetchElectricityBillInfo, payMonthlyElectricityBill } from '@/utils/paybillApi';
import {
    Select,
    SelectContent,
    SelectItem,
...
export default function ElectricityBill() {
    ...
    const getBillInfo = async () => {
        const respBillInfo = await fetchElectricityBillInfo()
        if (respBillInfo) {
    ...
    const handlePayment = async () => {
        const resp = await payMonthlyElectricityBill(month)
                ...
                <Card className="w-full min-h-screen mx-auto px-10">
                    <CardHeader className="flex flex-col sm:flex-row sm:justify-between sm:gap-4 sm:items-center sm:space-y-0">
                        <CardTitle className="text-2xl font-bold">
                            <Link href={"/dashboard"} className=' text-teal-500'>Dashboard</Link> / <Link href={"/paybill"}>Pay Bill</Link> / Electricity
                        </CardTitle>
